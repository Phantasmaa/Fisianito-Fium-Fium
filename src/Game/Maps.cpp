...
void Maps::createTeleport()
{
    teleport = new EntityNode();
    EntityNode* head = teleport;


    Entity teleport1, teleport2;

    teleport1.initAttributes(200, 10, 30, 60);
    teleport1.initShape();
    teleport2.initAttributes(300, 10, 15, 15);
    teleport2.initShape();


    head->value = teleport1;
    head->next_node = new EntityNode();
    head = head->next_node;
    head->value = teleport2;
    head->next_node = new EntityNode();
    head = head->next_node;
}
void Maps::initTeleport()
{
    EntityNode* head = teleport;
    while (head)
    {
...
void Maps::renderTeleport(sf::RenderTarget* target)
{
    EntityNode* head = teleport;
    while (head)
    {
